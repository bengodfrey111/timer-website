...
class timer:
    def __init__(self, name):
        ...
        cursor = connection.execute("SELECT password, password_salt, endTime FROM timers WHERE name=:name", {"name": name})
        for row in cursor:
            self.exist = True
            ...
            self.password = row[0]
            self.password_salt = row[1]
        connection.close()
    def controlTimer(self, password):
        return self.password == hashPassword(self.password_salt, password)
...
def newTimer(name, password): #creates the timer
    salt = uuid.uuid4().hex
    connection = sqlite3.connect(databaseLoc())
    connection.execute("INSERT INTO timers (name, password, password_salt, endTime) VALUES (:name, :password, :password_salt, :endTime)", {"name": name, "password": hashPassword(salt, password), "password_salt": salt, "endTime": math.ceil(time.time() * 1000)}) #datetime is stored as unix time to prevent timezone issues
    connection.commit()
    connection.close()
    return timer(name)
